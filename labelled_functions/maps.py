...
def parse_map_input(input_names, *args, **kwargs):
...
def lmap(f, *args, **kwargs):
    ...
    return map(f, *parse_map_input(f.input_names, *args, **kwargs))

def recorded_map(f, *args, **kwargs):
    f = LabelledFunction(f)
    return map(f.recorded_call, *parse_map_input(f.input_names, *args, **kwargs))
